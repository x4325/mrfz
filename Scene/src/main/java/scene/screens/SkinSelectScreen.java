...
public class SkinSelectScreen implements ISubscriber, CustomSavable<Integer> {
    ...
    private static final String SAVE_KEY = "scene_skin";
    private static final String UI_ID = SceneMod.makeID("SkinSelect");
    private static final String CHAR_PREFIX = SceneMod.imgPath("char/");
    private static final float SPINE_SCALE = 1.75f;

    public static final String[] SKIN_PATHS = {
            "idle/char_336_folivo",
            "skins/char_336_folivo_epoque_22/char_336_folivo_epoque_22",
            "skins/char_336_folivo_wild_11/char_336_folivo_wild_11",
    };
    ...
    public void refresh() {
        ...
        if (AbstractDungeon.player instanceof Scene) {
            ((Scene) AbstractDungeon.player).refreshSkin();
        }
    }
    ...
    private void updateInput() {
        if (CardCrawlGame.chosenCharacter != ClassEnum.SCENE) {
            return;
        }
        leftHb.update();
        rightHb.update();
