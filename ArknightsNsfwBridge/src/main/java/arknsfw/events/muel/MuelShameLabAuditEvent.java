...
public class MuelShameLabAuditEvent extends AbstractArkRouteEvent {
    public static final String ID = ArkNsfwMod.makeID("MuelShameLabAudit");

    public MuelShameLabAuditEvent() {
        super(ID, ArkEventImages.path(ArkEventImages.MUEL_SHAME_ACT1), ArkRunProgress.Route.SHAME, 4);
    }
    ...
    protected void onFirstChoice(int buttonUsed) {
        lockThisRoute();
        if (buttonUsed == 0) {
            ArkDebuffHelper.apply(AbstractDungeon.player, new BubbleGagPower(AbstractDungeon.player, 1));
        ...
        if (buttonUsed == 3) {
            ArkRouteDirtyOptionHelper.applyShameSelfHumiliate(
                    AbstractDungeon.player, new BubbleGagPower(AbstractDungeon.player, 2), 1);
            showResult(4);
        }
    }
}
