...
public class MuelFallFluidOverflowEvent extends AbstractArkRouteEvent {
    public static final String ID = ArkNsfwMod.makeID("MuelFallFluidOverflow");

    public MuelFallFluidOverflowEvent() {
        super(ID, ArkEventImages.path(ArkEventImages.MUEL_FALL_ACT3), ArkRunProgress.Route.FALL, 4);
    }
    ...
    protected void onFirstChoice(int buttonUsed) {
        ...
        if (buttonUsed == 3) {
            ArkRouteDirtyOptionHelper.applyMuelFallDirty(3);
            showResult(4);
        }
    }
}
