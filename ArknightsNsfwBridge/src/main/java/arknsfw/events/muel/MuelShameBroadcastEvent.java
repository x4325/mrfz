...
public class MuelShameBroadcastEvent extends AbstractArkRouteEvent {
    public static final String ID = ArkNsfwMod.makeID("MuelShameBroadcast");

    public MuelShameBroadcastEvent() {
        super(ID, ArkEventImages.path(ArkEventImages.MUEL_SHAME_ACT3), ArkRunProgress.Route.SHAME, 4);
    }
    ...
    protected void onFirstChoice(int buttonUsed) {
        lockThisRoute();
        if (buttonUsed == 0) {
            ArkDebuffHelper.apply(AbstractDungeon.player, new LeakPower(AbstractDungeon.player, 2));
        ...
        if (buttonUsed == 3) {
            ArkRouteDirtyOptionHelper.applyShameSelfHumiliate(
                    AbstractDungeon.player, new LeakPower(AbstractDungeon.player, 3), 3);
            showResult(4);
        }
    }
}
