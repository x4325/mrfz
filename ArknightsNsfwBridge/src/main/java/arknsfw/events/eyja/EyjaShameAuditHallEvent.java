...
public class EyjaShameAuditHallEvent extends AbstractArkRouteEvent {
    public static final String ID = ArkNsfwMod.makeID("EyjaShameAuditHall");

    public EyjaShameAuditHallEvent() {
        super(ID, ArkEventImages.path(ArkEventImages.EYJA_SHAME_ACT3), ArkRunProgress.Route.SHAME, 4);
    }
    ...
    protected void onFirstChoice(int buttonUsed) {
        ...
        if (buttonUsed == 2) {
            ArkDebuffHelper.apply(AbstractDungeon.player, new CoreStrainPower(AbstractDungeon.player, 2));
        ...
        if (buttonUsed == 3) {
            ArkRouteDirtyOptionHelper.applyShameSelfHumiliate(
                    AbstractDungeon.player, new AshShamePower(AbstractDungeon.player, 3), 3);
            showResult(4);
        }
    }
}
