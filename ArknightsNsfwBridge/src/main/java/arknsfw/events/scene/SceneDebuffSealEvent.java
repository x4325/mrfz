...
public class SceneDebuffSealEvent extends AbstractImageEvent {
    ...
    public SceneDebuffSealEvent() {
        super(
                ...
                ArkNsfwMod.makeImagePath("events/event_scene_debuff_seal.png")
        );
    ...
    protected void buttonEffect(int buttonUsed) {
        if (phase == 0) {
            if (buttonUsed == 0) {
                if (!ArkDebuffHelper.playerHasDebuff(ExposedLensPower.POWER_ID)) {
                    ArkDebuffHelper.applyAndLock(AbstractDungeon.player, new ExposedLensPower(AbstractDungeon.player, 2));
                } else {
                    ArkDebuffHelper.lockPermanent(ExposedLensPower.POWER_ID);
                    AbstractDungeon.player.getPower(ExposedLensPower.POWER_ID).updateDescription();
                }
