...
public class SkinSelectScreen implements ISubscriber, CustomSavable<Integer> {
    ...
    private static final String SAVE_KEY = "highmore_skin";
    private static final String UI_ID = HighmoreMod.makeID("SkinSelect");
    private static final String CHAR_PREFIX = HighmoreMod.imgPath("char/");
    private static final float SPINE_SCALE = 1.75f;

    public static final String[] SKIN_PATHS = {
            "idle/char_4066_highmo",
            "skins/char_4066_highmo_epoque_45/char_4066_highmo_epoque_45",
            "skins/char_4066_highmo_nian_10/char_4066_highmo_nian_10",
    };
    ...
    public void refresh() {
        ...
        if (AbstractDungeon.player instanceof Highmore) {
            ((Highmore) AbstractDungeon.player).refreshSkin();
        }
    }
    ...
    private void updateInput() {
        if (CardCrawlGame.chosenCharacter != ClassEnum.HIGHMORE) {
            return;
        }
        leftHb.update();
        rightHb.update();
