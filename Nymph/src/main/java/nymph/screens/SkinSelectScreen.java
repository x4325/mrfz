...
public class SkinSelectScreen implements ISubscriber, CustomSavable<Integer> {
    ...
    private static final String SAVE_KEY = "nymph_skin";
    private static final String UI_ID = NymphMod.makeID("SkinSelect");
    private static final String CHAR_PREFIX = NymphMod.imgPath("char/");
    private static final float SPINE_SCALE = 1.75f;

    public static final String[] SKIN_PATHS = {
            "idle/char_4146_nymph",
            "skins/char_4146_nymph_ambienceSynesthesia_6/char_4146_nymph_ambienceSynesthesia_6",
            "skins/char_4146_nymph_epoque_42/char_4146_nymph_epoque_42",
    };
    ...
    public void refresh() {
        ...
        if (AbstractDungeon.player instanceof Nymph) {
            ((Nymph) AbstractDungeon.player).refreshSkin();
        }
    }
    ...
    private void updateInput() {
        if (CardCrawlGame.chosenCharacter != ClassEnum.NYMPH) {
            return;
        }
        leftHb.update();
        rightHb.update();
