...
public class ArchettoBrokenString extends AbstractImageEvent {
    public static final String ID = ArchettoMod.makeID("BrokenString");

    public ArchettoBrokenString() {
        super(
                CardCrawlGame.languagePack.getEventString(ID).NAME,
                CardCrawlGame.languagePack.getEventString(ID).DESCRIPTIONS[0],
                ArchettoMod.imgPath("events/BrokenString.png"));
    ...
    protected void buttonEffect(int buttonPressed) {
        ...
        switch (buttonPressed) {
            case 0:
                AbstractDungeon.player.damage(new DamageInfo(AbstractDungeon.player, 4, DamageInfo.DamageType.HP_LOSS));
