...
public class ArchettoMusicHall extends AbstractImageEvent {
    public static final String ID = ArchettoMod.makeID("MusicHall");

    public ArchettoMusicHall() {
        super(
                CardCrawlGame.languagePack.getEventString(ID).NAME,
                CardCrawlGame.languagePack.getEventString(ID).DESCRIPTIONS[0],
                ArchettoMod.imgPath("events/MusicHall.png"));
    ...
    protected void buttonEffect(int buttonPressed) {
        ...
        switch (buttonPressed) {
            case 0:
                AbstractDungeon.actionManager.addToBottom(new ApplyPowerAction(AbstractDungeon.player, AbstractDungeon.player, new AimPower(AbstractDungeon.player, 2), 2));
