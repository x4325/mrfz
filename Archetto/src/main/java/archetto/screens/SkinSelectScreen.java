...
public class SkinSelectScreen implements ISubscriber, CustomSavable<Integer> {
    ...
    private static final String SAVE_KEY = "archetto_skin";
    private static final String UI_ID = ArchettoMod.makeID("SkinSelect");
    ...
    public void refresh() {
        ...
        if (AbstractDungeon.player instanceof Archetto) {
            ((Archetto) AbstractDungeon.player).refreshSkin();
        }
    }
    ...
    private void updateInput() {
        if (CardCrawlGame.chosenCharacter != ClassEnum.ARCHETTO) {
            return;
        }
        leftHb.update();
        rightHb.update();
